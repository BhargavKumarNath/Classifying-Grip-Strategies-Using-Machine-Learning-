...
def get_subject_df(_df):
    """Creates a subject lebel dataframe to avoid double counting"""
...
def plot_kinematic_distributions(_df, kinematic_vars):
    """Plots histogram and boxplots for key kinematic variables"""
...
def plot_pca_scatter(_df, features_for_pca):
    """Performs PCA and plots the first two components coloured by condition"""
    ...
    # Determine columns for colouring
    ...
    for i, hue_col in enumerate(hue_cols):
        ...
        axes[i].set_title(f"Coloured by {hue_col.title()}")
        axes[i].grid(True)
